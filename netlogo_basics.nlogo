...
citizens-own [arrived]

to setup

  clear-all

  create-citizens number_citizens ; to create a quantity (number_citizens) of turtles of breed citizens
  [set color one-of [blue red]
    set shape "person"
    set arrived false
    setxy random-xcor random-ycor]

  create-shops number_shops [
   set shape "house"
    set size 2
    ifelse random-float 1 < 0.5 [set color green][set color yellow]
    ifelse color = green [setxy -4 random-ycor][setxy 4 random-ycor]
  ]
...
to move
  ask citizens [ifelse not any? shops-on neighbors [fd 1 rt 360][set arrived true]]
end

to influence

if any? citizens-on neighbors [
ask one-of citizens-on neighbors [
...
An example to show basics of programming logic in NetLogo
...
Try to change the number of citizens or shops and order of commands
