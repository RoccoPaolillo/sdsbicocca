...
to update-opinion
  ask link-neighbors [
    if abs(opinion - [opinion] of myself) <= 1
    [set opinion (opinion + abs(([opinion] of myself - opinion)))]
    ]
end
...
to-report find-partner-color [colorturtle]
  report [one-of both-ends with [color = colorturtle]] of one-of links
end
...
64
NIL
ask n-of (count turtles * similars) turtles [set color green]
NIL
1
T
OBSERVER
...
163
test
ask turtle 30 [\nset like true\nask link-neighbors [set like true]]
NIL
1
T
OBSERVER
...
224
colorlike
ask turtles [\nifelse like = true \n[set color yellow][set color blue]\n]
NIL
1
T
OBSERVER
...
284
NIL
let sample beta-random 0.1 0.1\nprint sample
NIL
1
T
OBSERVER
